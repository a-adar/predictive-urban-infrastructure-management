...
st.write("This dashboard presents baseline and AI-controlled traffic performance across multiple scenarios.")

st.sidebar.title("Dashboard Controls")
view = st.sidebar.radio("Choose view", ["Overview", "Scenario Comparison", "Incident Analysis"])
...
if view == "Overview":
    ...
    - **AI system:** ML-driven adaptive controller  
...
# Section 3: Incident summary
    ...
    plot3 = Path("results/plots/incident_waiting_time_comparison.png")
...
if baseline_ai_file.exists():
    ...
    st.markdown(f"""
- **Lowest average waiting time:** {best_wait['system']} in {best_wait['scenario']} scenario ({best_wait['avg_waiting_time']:.2f})
- **Highest average waiting time:** {worst_wait['system']} in {worst_wait['scenario']} scenario ({worst_wait['avg_waiting_time']:.2f})
- Use this dashboard together with the report discussion to explain where the AI controller helps and where it still needs improvement.
""")